...
class MVChunkCache implements ChunkListener{
	 ...
	 * @return MVChunkCache
	 */
	public static function getInstance(World $world, Compressor $compressor, PacketTranslator $translator) : self{
		$worldId = spl_object_id($world);
		$compressorId = spl_object_id($compressor);
		$translatorId = spl_object_id($translator);
		if(!isset(self::$instances[$translatorId])){
			GlobalLogger::get()->debug("Created new chunk packet cache (world#$worldId, compressor#$compressorId, packetTranslator#$translatorId)");
			self::$instances[$translatorId] = [];
		}
		if(!isset(self::$instances[$translatorId][$worldId])){
			self::$instances[$translatorId][$worldId] = [];
		...
		if(!isset(self::$instances[$translatorId][$worldId][$compressorId])){
			self::$instances[$translatorId][$worldId][$compressorId] = new self($world, $compressor, $translator);
		}
		return self::$instances[$translatorId][$worldId][$compressorId];
	}
